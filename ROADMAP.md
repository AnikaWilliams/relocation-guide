...
Last updated: 2026-06-14 (Apple web design-language reskin (ADR-0022, supersedes ADR-0019; founder direction): the ENTIRE site (home, corridor page + no-JS guide, the plan island, legal/about/contact, shared chrome) reskinned to Apple's web design language — SF system-font stack (EB Garamond/Lato retired → no web-font fetch), Apple token set (label #1d1d1f, secondaryLabel #6e6e73, accent #0071e3, surface #f5f5f7, separator #d2d2d7), generous whitespace, rounded cards, pill primary buttons. Styling-only: zero content/YAML/provenance/logic changes; all trust UI preserved (Verified badge, Source + last-verified lines, disclaimer modal gate, journey map, canton panel, Sources & legal-notes fold, Impressum, route-not-covered + Russia notices). A11y: contrast lifted to AA (tertiaryLabel #6b6b70 = 5.30:1; placeholders → secondaryLabel 5.07:1), 44px hit targets, scalable type ramp (no hardcoded micro-px); apple-hig design-reviewer PASS. Dark mode deferred (tokens dark-ready). astro check 0 / build + gate (15 pages) / 281 tests; verified live (Playwright). Plain-language reading surface (founder mandate "no legal jargon"): all statute/act/article/SR-code citations (FNIA, ASEO, VZAE, OASA, "Art. 30 para. 1 let. b", "Arts. 42-44", SR 142.20, "concubinage") removed from us-ch's DEFAULT-VISIBLE prose (titles, summary/tldr, step text/tips, warnings, document descriptions) and rewritten in plain English — the entitlement-vs-discretion meaning preserved in substance; the law-named per-step source links moved into the collapsed "Sources & legal notes" fold (the actionable canton-office link stays inline); precise citations remain in keyFacts/detail/source links (the Reference/Sources folds), so provenance + "verified against official sources" hold. Two-agent: content-researcher rewrote → fact-verifier independently confirmed no jargon remains + meaning preserved + provenance untouched (VERIFICATION_LOG 2026-06-14); compliance PASS; astro check 0 / build+gate (15 pages) / 281 tests. Scope: us-ch; other 7 corridors' prose to follow. Interactive "journey map" + pure-state gamification (design-research-led; builds on the calmer-UI branch; stacked on it): the flat "Your journey" sidebar list is now a connected, chapter-grouped VISUAL PATH (chapters from an authored category→chapter map in `src/utils/journeyChapters.ts`: Before you go / Your permit / After you arrive / Settling in), each node carrying its real state (done/available/current/locked) from `statusFor()` + a per-node document meter from `docProgress()`; available nodes are clickable, LOCKED nodes stay readable (only Mark-done gated) and show the SPECIFIC "Unlocks after: …" prerequisite from `dependsOn`. Plus a per-task document meter on the card, an instant motivation micro-confirmation, a non-punitive "Welcome back" continuity line, and per-chapter milestone notes extending the existing momentum toast. Everything is a pure projection of existing state — NO dark patterns (no streaks/countdowns/points/false-approval), enforced by a unit test (`tests/journeyChapters.test.ts`) that BANS outcome/approval words in chapter copy. Disclaimer modal gate + provenance + Impressum + Russia/route-not-covered notices untouched; prefers-reduced-motion respected; keyboard-accessible nodes. Verified: astro check 0 / build+gate (15 pages) / 281 tests; a11y PASS (WCAG 2.2 AA audited); trust/dark-pattern review PASS. Deferred: endowed head-start pre-credit, wizard auto-advance, interactive canton map (needs sourced boundary geometry). Calmer/encouraging UI overhaul (design-research-led; pending human-lawyer sign-off): removed the gold "not legal advice" banner everywhere; after the intake a REQUIRED disclaimer modal gate (role=dialog, focus-trapped, no bypass, compliance-owned wording single-sourced in `src/utils/disclaimer.ts`, acked in localStorage `rg-disclaimer-ack-v1`) must be confirmed before the plan renders — also gates shared-link opens; task cards re-sequenced action-first (tldr→steps→documents) with key facts/legal citations/detail behind one "Reference" fold and the caveats consolidated into one "Sources & legal notes" fold (a short "not legal advice" line stays always-visible per card per compliance); plain-language intake options (dropped L/B/C permit codes & "concubinage"); stable phase-based progress ("Your route→situation→details") with progressbar a11y; warm all-breakpoint intro; quiet per-step momentum + calmer document gate. Disclaimer still in the modal + footer (every page) + per-card + Impressum + no-JS guide. Compliance verdict: CONDITIONAL — the modal-acknowledgement posture (vs a persistent banner) needs human-lawyer sign-off before merge; astro check 0 / build+gate / 277 tests green; a11y reviewed (modal focus-trap OK). Cantonal fee/timeline transparency: 12 us-ch steps that have no single federal fee/processing-time figure (the employer-sponsored work permit, the family-reunification routes, the retirement permit, the study permits/sub-steps) now show a user-facing "Fees & processing time" note that these are set by the canton — or, for study admission, the institution — and to confirm locally; driven by a presentation-only `localCostTimeline` task hint (no invented numbers, no new sourced claims, ADR-0007/ADR-0021), plus a "varies by canton" caveat now always shown in the "Your canton" panel. Canton-aware plan-step links: the six "apply at / contact your cantonal authority of the canton where you live/study" steps (family reunification ×3, unmarried-partner concubinage, retirement, study) now deep-link the user's *selected* canton's own migration office (ZH Migrationsamt, GE OCPM, VD SPOP, ZG, BS) instead of the federal SEM directory of all 26 — falling back to that directory for not-yet-sourced cantons, so no dead ends. A `StepLinkSchema.cantonOffice` rendering hint (asserts no fact, no provenance) drives a frontend-only swap to the already-VERIFIED canton migrationOffice claim; static SSR full-guide stays federal (can't know the canton). Build/gate/`astro check`/277 tests green; fact-verifier (accurate, no overreach) + provenance-integrity verified; ADR-0021. Outbound links: the written-guide's 109 external source links now open in a new tab (target=_blank + rel=noopener noreferrer), matching the planner — readers don't lose their place. Plan UI: the desktop plan sidebar (progress / answers / journey) is now collapsible (Hide/Show), giving the task panel full width; mobile already had this. Canton dimension for Switzerland (ADR-0021): corridor-embedded `cantons` schema + provenance gate; CH-only "which canton?" intake step (searchable combobox over 26 cantons, optional) + `cn` share-param + "Your canton" plan panel with a verified SEM-directory fallback; per-canton sourced detail for ZH/GE/VD/ZG/BS in the two-agent pipeline. Earlier: Link audit: all 950 source links render-checked — 147 good, 23 Fedlex "under preparation" links flagged + the us-ch fix applied (79 occurrences → static-HTML, independently fact-verifier re-verified; re-audit now 0 suspect / 170 good). The weekly link-auditor now also checks anchor-reach — a `#article` link that loads but whose section never renders (SPA shell) goes RED on SPA domains (fedlex-class regressions); france-visas added to manual-check (403s the bot, fine for users). Live run: 0 errors / 142 URLs. Plan-personalisation correctness: "Who's joining you?" companion multi-select (partner/children/"It's just me") replaces the children yes/no; family/dependant steps gated to who is actually coming; full-guide route labels; cross-corridor plan-correctness regression suite — ADR-0020. UI/UX "Trust & Authority" design system adopted — audit fixes 1–10: AA-contrast CTAs, 44px touch targets, readable legal text, EB Garamond + Lato self-hosted fonts, brand tokens, consent-overlap fix, home/wizard trust framing, plus polish 11–13 (SVG icons, consistent focus rings, tighter line-length); ADR-0019. Earlier: 8 corridors published + intake availability dots, ADR-0018; source-snapshot cache, ADR-0017; corridor production via agent teams in two runs, ADR-0016; all-routes intake + launch waitlist, ADR-0015; launch-readiness drafts, og:image, dormant consent+analytics, ADR-0013; us-ch founder approval, ADR-0014; go-live blocked on domain + lawyer review)
...
- [~] **Canton-aware path shaping (Switzerland)**: replace the free-text "canton or city" intake field with a canton picker, and let the chosen canton shape next steps — link the exact cantonal migration office, and (longer-term) reinstate the 5 cantonal-variable fee/timeline fields omitted under ADR-0007 as per-canton verified claims. _Content-heavy: every per-canton fact needs the full two-agent pipeline (26 cantons), so machinery first, cantons added incrementally starting with Zürich/Geneva/Vaud/Basel-Stadt/Zug._ **Done (ADR-0021):** canton picker + intake `cn` param + "Your canton" panel; and the exact cantonal migration office is now deep-linked both in that panel **and** in the plan's "contact your cantonal authority" steps (`StepLinkSchema.cantonOffice`, 2026-06-14). Sourced cantons: ZH/GE/VD/ZG/BS (others fall back to the federal SEM directory). Where no federal fee/processing-time figure exists, the plan now **notes** that these are set by the canton (or, for study admission, the institution) and to confirm locally (`localCostTimeline` hint, 2026-06-14) — transparency without invented numbers. **Still pending:** source the actual per-canton fee/timeline figures, and expand canton coverage beyond the first five.
...
- [x] **Re-verified the 62 re-sourced claims** (separate fact-verifier pass, 2026-06-13, two-agent rule): **60 → VERIFIED, 2 still open**. Batch now **730/732 VERIFIED; 7 of 8 corridors 100% clean** (us-de, us-fr, us-be, us-ie, us-lu, us-at, us-gb). us-nl is 90/92 — 2 open in `health-insurance-zvw` (Zvw deadline stated two ways by two official sources: arrival-based vs permanent-permit-based — needs human/lawyer reconciliation, not re-sourcing). Gate green; all `published: false`.
- [x] **Founder publish gate — 7 corridors published 2026-06-13 (ADR-0018):** `us-de, us-fr, us-be, us-ie, us-lu, us-at, us-gb` set `published: true` (+ `us-ch` = 8 live). Build gate confirmed all 8 fully VERIFIED + in-date (renders 15 pages). Intake now shows green/gray availability dots per route. _Public go-live still needs the production domain + legal-page lawyer review (same as us-ch)._
- [ ] **us-nl:** reconcile the Zvw 4-month deadline conflict (founder/lawyer call) → re-verify those 2 → then us-nl is clean.
