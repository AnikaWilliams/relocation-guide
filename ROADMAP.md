...
- [ ] **USA → Western Europe + UK batch via agent teams (ADR-0016, 2026-06-13):** `us-de, us-fr, us-nl, us-ie, us-at, us-be, us-lu, us-gb`, on branch `corridor/us-weu-research`. Run 1 (research) — **6 of 8 drafted 2026-06-13** (`us-de` 14t/94c, `us-fr` 11t/111c, `us-nl` 10t/92c, `us-at` 11t/91c, `us-be` 7t/50c, `us-ie` 11t/84c = **522 claims, all UNVERIFIED, `published: false`**, sources captured per corridor). `us-lu` + `us-gb` sources captured but YAML not yet written (team ended over a long pause) → finishing pass in progress. **Run 2 = a SEPARATE `fact-verifier` team is REQUIRED before any claim is VERIFIED or any corridor is published** — research output is not publishable on its own. Opus 4.8 throughout; official sources captured via the ADR-0017 snapshotter (local `curl`, browser UA).
